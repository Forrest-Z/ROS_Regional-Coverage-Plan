...
#include <costmap_2d_kinetic/cost_values.h>
...
CleaningPathPlanning::CleaningPathPlanning(costmap_2d::Costmap2DROS *costmap2d_ros)
{
    ...
    int sizex = costmap2d_->getSizeInCellsX();//Accessor for the x size of the costmap in cells.
    ...
    // cv::Mat image = cv::imread("/home/zq/catkin_ws/src/tx2_navigation/maps/mymap2.pgm",0);
...
void CleaningPathPlanning::getCellMatAndFreeSpace(Mat srcImg, Mat &cellMat,vector<cellIndex> &freeSpaceVec)
{
    ...
    bool isFree = true;
    int r = 0,c = 0, i = 0,j = 0;
    //check the cost of cell in costmap
    ...
    for (r = 0;r < cellMat.rows; r++ )
    {
        for(c = 0 ; c < cellMat.cols; c++)
        {
            ...
            // int rt = costmap_2d::LETHAL_OBSTACLE;
            // cout<<"the LETHAL_OBSTACLE_space is :"<<rt<<endl;
        }
    }
...
void CleaningPathPlanning::writeResult(Mat resultmat,vector<cellIndex> pathVec)
{
    ...
    for(i = 1; i<pathVec.size();i++)
    {
        ...
        {
            line(resultmat,initpoint,cupoint,Scalar(0,255,0),1,8);
        }
        else line(resultmat,initpoint,cupoint,Scalar(0,0,255),1);
        initpoint = cupoint;
...
//    initPoint.col = cellMat_.cols/2;
    ...
    std::cout<<"the mx is:"<<mx<<"&&&&&&&&&&&&&"<<"the my is:"<<my<<std::endl;
    if(!getmapcoor)
    {
    ...
    float initTheta = initPoint.theta; //initial orientation 90
...
bool CleaningPathPlanning::initializeCoveredGrid()
{
    ...
    // cout<<"the world oringin x is: "<<wx<<"  "<<"the world oringin y is: "<<wy<<endl;;
